...
div.d-flex.flex-row(style="height: calc(100%)")
    v-list(:selected="[selected_student_id]" mandatory style="min-width: 155px; overflow-y: scroll")
        v-list-item Leerlingen
        v-divider
        v-list-item(v-for="(student, index) in test.students" :value="student.id" @click="$emit('update:selected_student_id', student.id)" append :title="student.student_id")
            template(v-slot:append)
                v-icon(
                    color="red"
                    icon="mdi-delete"
                    @click="test.students.splice(index, 1)"
                )
    v-divider(vertical)
