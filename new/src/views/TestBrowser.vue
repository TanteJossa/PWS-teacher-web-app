                ...
                v-card-text
                    ...
                    v-list(v-else)
                        v-list-item(v-for="test in publicTests" :key="test.id" @click="openTest(test)" :title="test.name")
                            v-list-item-subtitle Updated at: {{ new Date(test.updated_at).toLocaleDateString() }}
...
export default {
    ...
    computed: {
        userTests(){
            return this.testManager.tests.filter(test => test.user_id == this.userStore.user.uid);

        },
        publicTests() {
            return this.testManager.tests.filter(test => test.is_public && test.user_id != this.userStore.user.uid);
        },
        filteredAdminTests() {
        ...
        async fetchTests() { // Changed method name to prevent conflict with TestManager
            ...
            // try {
                ...
                if (this.userStore.user.role == "admin") {
                    ...
                    q = query(collection(db, "tests"), or(where("user_id", "==", this.userStore.user.uid), where( "is_public", "==", true)), limit(100));
                }
